...
const plugin = {
  ...
  register(api) {
    ...
    // Default workspace from config or gateway process cwd
    ...
    function storeSessionRoute(sessionId, context) {
      sessionRoutes.set(sessionId, {
        channelId: context.channelId,
        userId: context.userId,
        accountId: context.accountId || context.config?.accountId
      });
    }
    ...
    function registerCommands(api) {
      ...
      api.registerCommand({
        ...
        execute: async (context) => {
          const prompt = context.input?.trim();
          if (!prompt) {
          ...
          storeSessionRoute(sessionId, context);
      ...
      api.registerCommand({
        ...
        description: '查看会话状态',
        execute: async (context) => {
          const sessionId = bridge.findActiveSession(context.userId);
          if (!sessionId) {
      ...
      api.registerCommand({
        ...
        execute: async (context) => {
          const answer = context.input?.trim();
          if (!answer) {
      ...
      api.registerCommand({
        ...
        description: '批准审批请求',
        execute: async (context) => {
          const shortId = context.input?.trim();
          ...
          if (meta && meta.senderId !== context.userId) {
